...
namespace nexus {
  ...
  void Next100MuonVeto::Construct()
  {
    ...
    //G4_PLASTIC_SC_VINYLTOLUEN
    //G4_POLYSTYRENE  
      
    G4LogicalVolume* wallr_box_logic = new G4LogicalVolume(wallr_box_solid,mat,"MUON_VETO_RWALL");
    G4LogicalVolume* walll_box_logic = new G4LogicalVolume(walll_box_solid,mat,"MUON_VETO_LWALL");
    G4LogicalVolume* doorf_box_logic = new G4LogicalVolume(doorf_box_solid,mat,"MUON_VETO_FDOOR");
    G4LogicalVolume* doorb_box_logic = new G4LogicalVolume(doorb_box_solid,mat,"MUON_VETO_BDOOR");
    G4LogicalVolume* roof_box_logic  = new G4LogicalVolume(roof_box_solid,mat,"MUON_VETO_ROOF");

    // location of planes w.r.t. to mother volume

    new G4PVPlacement(0, G4ThreeVector(door_x_/2-wall_x_/2,0,0.),
                      wallr_box_logic, "MUON_VETO_RWALL", veto_box_logic, false, 0, false);
    new G4PVPlacement(0, G4ThreeVector(-door_x_/2+wall_x_/2,0,0.),
                      walll_box_logic, "MUON_VETO_LWALL", veto_box_logic, false, 0, false);
    new G4PVPlacement(0, G4ThreeVector(0.,0.,wall_z_/2-door_z_/2),
                      doorf_box_logic, "MUON_VETO_FDOOR", veto_box_logic, false, 0, false);
    new G4PVPlacement(0, G4ThreeVector(0.,0.,-wall_z_/2+door_z_/2),
                      doorb_box_logic, "MUON_VETO_BDOOR", veto_box_logic, false, 0, false);
    new G4PVPlacement(0, G4ThreeVector(0.,wall_y_/2-roof_y_/2,0.),
                      roof_box_logic, "MUON_VETO_ROOF", veto_box_logic, false, 0, false);

    //Set the volume sensitive detector. 

    wallr_box_logic->SetUserLimits(new G4UserLimits(max_step_size_));
    walll_box_logic->SetUserLimits(new G4UserLimits(max_step_size_));
    doorf_box_logic->SetUserLimits(new G4UserLimits(max_step_size_));
    doorb_box_logic->SetUserLimits(new G4UserLimits(max_step_size_));
    roof_box_logic->SetUserLimits(new G4UserLimits(max_step_size_));
  ...
  G4ThreeVector Next100MuonVeto::GetDimensions() const
  {
    return G4ThreeVector(door_x_/2., wall_y_/2., wall_z_/2.);
  }
